...
var GMAP_KEY = 'AIzaSyBom-cjRo-fy7JXK7UcX--BhKweCRXCGSM';
var GMAP_BASE_URL = 'http://maps.googleapis.com/maps/';

// AIzaSyBom-cjRo-fy7JXK7UcX--BhKweCRXCGSM

var YELP_BASE_URL = 'http://api.yelp.com/v2/search/';
var YELP_CONSUMER_KEY = 'QDjUDjcyJm01dz_NPhUlRw';
var YELP_CONSUMER_SECRET = 'rKa0IsIz0xRnA1PpKdi_DMsIsWQ';
var YELP_TOKEN = 'tJL7VrTzLMTY2oLMLNnccRx2_sqtjgLe';
var YELP_TOKEN_SECRET = 'GO3wgsHu58M67PwuiQMtjtUkFPc';
...
var CURRENT_LOCATION = { lat: 38.900138, lng: -77.044532 };
...
function createMarker(location) {
  ...
  var content = '<div id="venue">' + '<h5 class="venue-name">' + (location.name || '') + '</h5>' + '<div class="venue-address">' + (location.address[0] || '') + '<br>' + (location.address[2] || location.address[1] || '') + '</div>' + '<div class="venue-contact">' + (location.phone || '') + '</div>' + '<div class="venue-url"><a href="' + parseURL + '</a></div>' + '<div class="venue-rating" style="margin-top:10px"><img src="' + (location.ratingImage || '') + '" /></div>' + '</div>';
...
function getYelpData(locationArray) {
  ...
  $.each(locationArray, function (index, location) {
    ...
    params.oauth_signature = oauthSignature.generate('GET', YELP_BASE_URL, params, YELP_CONSUMER_SECRET, YELP_TOKEN_SECRET);
